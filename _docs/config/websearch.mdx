...
1. **Local adapters** (`websearch.provider`) — crabcode runs a `websearch` tool against Exa, Tavily, Brave, TinyFish, Monid, Ollama Cloud, etc.
...
**Local adapters** bill (or rate-limit) through their own APIs — `exa-hosted-mcp` and `firecrawl-hosted-mcp` are keyless/rate-limited; Exa/Tavily/Brave/TinyFish/Monid/Ollama Cloud/etc. use your key (TinyFish Search and Monid→TinyFish Search are free usage).
...
| `tinyfish` | `TINYFISH_API_KEY` | TinyFish Search API (`api.search.tinyfish.ai`) — free with a TinyFish account |
| `monid` | `MONID_API_KEY` | Monid catalog → TinyFish `/search` (no TinyFish key; uses your Monid API key) |
